...
// Constants
// ---------------------------------------------------------------------------
export const SLA_KILLSWITCH_RULE_ID = "SLA_KILLSWITCH_V1";
...
// Rule implementation
// ---------------------------------------------------------------------------
export const slaKillswitchRule = {
    id: SLA_KILLSWITCH_RULE_ID,
    name: "SLA Kill Switch",
    version: "1.0.0",
    description: "Detects when previously deferred decisions have exceeded the SLA window " +
    ...
    evaluate(ctx) {
        ...
        const isExplicitSignal = event.type === PAYMENT_FAILED &&
            event.payload["slaExceeded"] === true;
        ...
        const alreadyPenalized = new Set(state.riskEvents
        ...
        if (hasBreach) {
            ...
            return {
                trace,
                verdictContribution: flagContribution(isExplicitSignal
        ...
        const trace = buildTrace(traceId, ctx, "no_match", `No SLA breaches. ${deferredDecisions.length} deferred decision(s) within the ${SLA_THRESHOLD_MS / 1000}s window.`, conditions, actions);
