...
export const TIMEZONE_RISK_RULE_ID = "TIMEZONE_RISK_V1";
...
// ---------------------------------------------------------------------------
function getLocalHour(timezone, isoTimestamp) {
    try {
...
function resolveHour(ctx, timezone) {
...
// Rule implementation
// ---------------------------------------------------------------------------
export const timezoneRiskRule = {
    id: TIMEZONE_RISK_RULE_ID,
    name: "Timezone Risk",
    version: "1.1.0",
    description: "Flags events that occur during abnormal local hours — payments between " +
    ...
    evaluate(ctx) {
        ...
        if (isUserEvent) {
            ...
            if (isSuspicious) {
                ...
                const trace = buildTrace(traceId, ctx, "fired", `FLAGGED: Login timezone mismatch — ${hourDelta}h delta (device: ${deviceTimezone}, account: ${accountTimezone}).`, conditions, actions);
                return {
                    trace,
                    verdictContribution: flagContribution(`Timezone mismatch: ${hourDelta}h delta between device (${deviceTimezone}) and account (${accountTimezone})`),
                };
            }
            actions.push({
            ...
            const trace = buildTrace(traceId, ctx, "no_match", `Timezone delta ${hourDelta ?? "unknown"}h — within acceptable range. No risk.`, conditions, actions);
        ...
        const isAbnormal = localHour !== null &&
        ...
        if (isAbnormal) {
            ...
            const trace = buildTrace(traceId, ctx, "fired", `Payment at local hour ${localHour} (${timezone}) is outside normal window. Risk flagged.`, conditions, actions);
            return {
                trace,
                verdictContribution: flagContribution(`Payment at unusual local hour ${localHour} in timezone ${timezone}`),
            };
        }
        actions.push({
            actionType: "no_action",
        ...
        const trace = buildTrace(traceId, ctx, "no_match", `Payment at local hour ${localHour ?? "unknown"} (${timezone}) — within normal window.`, conditions, actions);
        return { trace, verdictContribution: allowContribution() };
    },
};
