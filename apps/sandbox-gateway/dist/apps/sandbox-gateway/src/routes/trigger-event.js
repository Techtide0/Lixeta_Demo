...
import { createEventSource, createDomainEvent } from "@lixeta/models";
...
function fail(field, msg) {
    return { success: false, error: { flatten: () => ({ fieldErrors: { [field]: [msg] }, formErrors: [] }) } };
}
const triggerEventSchema = {
    safeParse(raw) {
        if (!isPlain(raw))
            return { success: false, error: { flatten: () => ({ fieldErrors: {}, formErrors: ["Body must be a JSON object"] }) } };
        ...
        if (!isValidSessionIdFormat(sessionId.trim()))
            return fail("sessionId", "Invalid format — must start with sess_");
        if (typeof type !== "string" || !type.trim() || type.length > 128)
            return fail("type", "Required non-empty string (max 128 chars)");
        if (!isPlain(payload))
            return fail("payload", "Required JSON object");
        if (source !== undefined) {
            if (!isPlain(source))
        ...
        const data = {
            ...
            type: type.trim(),
            payload: payload,
            ...(timestamp !== undefined && typeof timestamp === "string" ? { timestamp: timestamp } : {}),
            ...(source !== undefined ? { source: source } : {}),
            ...(correlationId !== undefined && typeof correlationId === "string" ? { correlationId: correlationId } : {}),
        };
...
router.post("/", async (req, res) => {
    ...
    if (!sessionResult.ok) {
        if (sessionResult.code === "SESSION_NOT_FOUND" || sessionResult.code === "SESSION_EXPIRED") {
            send404(res, requestId);
            return;
        }
        if (["SESSION_COMPLETED", "SESSION_PAUSED", "SESSION_ERROR_STATE"].includes(sessionResult.code)) {
            send409(res, sessionResult.message, requestId);
            return;
        }
    ...
    const src = body.source
        ? createEventSource({ ...body.source, channel: body.source.channel })
        : createEventSource({ id: "api-caller", name: "API Caller", version: "1.0.0", channel: "api" });
    let event;
    try {
    ...
    if (!appendResult.ok) {
        if (appendResult.code === "SESSION_COMPLETED" || appendResult.code === "SESSION_ERROR_STATE") {
            send409(res, appendResult.message, requestId);
            return;
        }
        if (appendResult.code === "SESSION_NOT_FOUND" || appendResult.code === "SESSION_EXPIRED") {
            send404(res, requestId);
            return;
        }
        send500(res, requestId);
