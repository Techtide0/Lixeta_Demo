...
router.get("/", (req, res) => {
    ...
    if (!isValidSessionIdFormat(sessionId.trim())) {
        send400(res, "Invalid sessionId format. Session IDs start with 'sess_' followed by alphanumeric characters.", requestId);
        return;
    }
    ...
    const revenueEvents = state.revenueEvents.filter((r) => paginatedEventIds.has(r.triggeringEventId));
    const riskEvents = state.riskEvents.filter((r) => paginatedEventIds.has(r.triggeringEventId));
    const ruleTraces = state.ruleTraces.filter((t) => paginatedEventIds.has(t.triggeringEventId));
    const decisions = [];
...
// Helper
// ---------------------------------------------------------------------------
function parseIntParam(raw, defaultValue, min, max) {
