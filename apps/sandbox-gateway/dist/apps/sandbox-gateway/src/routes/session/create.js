...
function validateMetadata(raw) {
    ...
    if (tags !== undefined) {
        if (!Array.isArray(tags))
            return { ok: false, error: "metadata.tags must be an array" };
        if (tags.length > MAX_TAGS)
            return { ok: false, error: `metadata.tags must not exceed ${MAX_TAGS} items` };
    ...
    return {
        ...
        data: {
            ...
            ...(tags !== undefined ? { tags: tags.slice() } : {}),
        },
    };
}
function validateTtl(raw) {
    if (raw === undefined)
        return { ok: true, data: 2 * 60 * 60 }; // default 2 hours
...
function validateScenarioField(raw, field, maxLen) {
    if (raw === undefined)
