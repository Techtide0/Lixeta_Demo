...
import { computeSessionAnalytics, computeMultiSessionAnalytics, } from "../analytics/engine.js";
...
router.get("/", (req, res) => {
    ...
    if (typeof rawParam !== "string" || rawParam.trim() === "") {
        send400(res, "Missing required query parameter: sessionId. " +
            "Provide a single session ID or comma-separated list (max 10).", requestId);
        return;
    }
    ...
    if (sessionIds.length > MAX_SESSIONS_PER_REQUEST) {
        send400(res, `Too many sessions. Maximum ${MAX_SESSIONS_PER_REQUEST} per request. ` +
            `Received ${sessionIds.length}.`, requestId);
        return;
    }
    // ── Validate formats ──────────────────────────────────────────────────────
    const invalidIds = sessionIds.filter((id) => !isValidSessionIdFormat(id));
    if (invalidIds.length > 0) {
        send400(res, `Invalid session ID format: ${invalidIds.join(", ")}. ` +
            "Session IDs must start with 'sess_'.", requestId);
        return;
    }
    // ── Fetch sessions ────────────────────────────────────────────────────────
    const found = [];
    ...
    try {
        if (found.length === 1 && notFound.length === 0) {
            ...
            const item = found[0];
            const analytics = computeSessionAnalytics(item.sessionId, item.sessionStatus, item.state);
            const failedMetrics = analytics["failedMetrics"];
            ...
            const allFailedMetrics = new Set();
            for (const session of analytics.sessions || []) {
                const failed = session["failedMetrics"];
                if (failed) {
