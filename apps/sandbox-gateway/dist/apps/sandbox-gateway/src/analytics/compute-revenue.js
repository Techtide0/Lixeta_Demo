...
// Main computation
// ---------------------------------------------------------------------------
export function computeRevenue(revenueEvents, minorUnitMultiplier = 100) {
    ...
    const events = revenueEvents.filter((e) => e.amount.currency === primaryCurrency);
    let totalGain = 0;
    let totalLoss = 0;
    let totalSavings = 0;
    let gainCount = 0;
    ...
    for (const event of events) {
        ...
        const evtType = event.triggeringEventType;
    ...
    const byChannel = {};
    for (const [ch, net2] of channelMap)
        byChannel[ch] = net2;
    const byEventType = {};
    for (const [et, net2] of eventTypeMap)
        byEventType[et] = net2;
    const byDay = [];
    ...
    const breakdown = { byCategory, byChannel, byEventType, byDay };
    return {
...
// Helpers
// ---------------------------------------------------------------------------
function getPrimaryCurrency(events) {
...
function emptyRevenue(multiplier) {
    return {
