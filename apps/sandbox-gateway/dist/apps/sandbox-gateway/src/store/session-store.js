...
import { applyEngineOutput, recordAccess, markCompleted, } from "./session-record.js";
...
function storeErr(code, message) {
...
export function createSession(options = {}) {
    ...
    if (_store.size >= MAX_SESSIONS) {
        return storeErr("CAPACITY_EXCEEDED", `Session capacity limit (${MAX_SESSIONS}) reached. ` +
 ...
 */
export function updateAggressionLevel(sessionId, level) {
    const result = getSession(sessionId);
    if (!result.ok)
        return result;
    if (result.value.status === "completed") {
        return storeErr("SESSION_COMPLETED", `Session "${sessionId}" is closed. Aggression cannot be changed.`);
    }
...
export function getSession(sessionId) {
    ...
    if (isExpired(record, now)) {
        ...
        return storeErr("SESSION_EXPIRED", `Session "${sessionId}" has expired. Create a new session.`);
    }
    return storeOk(record);
}
/**
 ...
 */
export function appendEventToSession(sessionId, event, output) {
    const now = Date.now();
    gcSweep(now);
    const record = _store.get(sessionId);
    if (record === undefined) {
    ...
    if (isExpired(record, now)) {
        _store.delete(sessionId);
        return storeErr("SESSION_EXPIRED", `Session "${sessionId}" has expired. Events cannot be added to expired sessions.`);
    }
    if (record.status === "completed") {
        return storeErr("SESSION_COMPLETED", `Session "${sessionId}" has been closed. Create a new session to continue.`);
    }
    if (record.status === "paused") {
        return storeErr("SESSION_PAUSED", `Session "${sessionId}" is paused. Resume it before sending new events.`);
    }
    if (record.status === "error") {
        return storeErr("SESSION_ERROR_STATE", `Session "${sessionId}" is in an error state. ` +
...
export function closeSession(sessionId) {
    ...
    if (!result.ok)
        return result;
    if (result.value.status === "completed") {
        return storeErr("SESSION_COMPLETED", `Session "${sessionId}" is already closed.`);
    }
...
export function _injectSessionForTesting(record) {
    if (process.env["NODE_ENV"] === "production") {
