 ...
 */
import { buildEngineConfig, registerBuiltinRules, freezeRegistry, ALL_BUILTIN_RULE_IDS, } from "@lixeta/rules-engine";
let _engineConfig = null;
/**
 * Build and cache the EngineConfig.
 * Registers built-in rules and freezes the registry on first call.
 */
export function getEngineConfig(env) {
    ...
    freezeRegistry();
    const enabledRuleIds = env.ENABLED_RULE_IDS.length > 0
    ...
    _engineConfig = buildEngineConfig({
        ...
        security: {
            ...
            trustedSourceIds: env.TRUSTED_SOURCE_IDS.length > 0
...
export function _resetEngineConfigForTesting() {
