...
function assertNodeEnv(value) {
    if (value === "development" || value === "test" || value === "production") {
        return value;
    }
    throw new Error(`[Config] NODE_ENV must be "development", "test", or "production". Got: "${value}"`);
}
// ---------------------------------------------------------------------------
// Validation
// ---------------------------------------------------------------------------
function validateEnv(env) {
    ...
    if (env.NODE_ENV === "production" &&
        ...
        throw new Error("[Config] ALLOWED_ORIGINS cannot contain '*' in production. " +
            "Set explicit origin allowlist.");
    }
    // Auth must be enabled in production
    if (env.NODE_ENV === "production" && !env.REQUIRE_AUTH) {
        throw new Error("[Config] REQUIRE_AUTH must be true in production. Set API_KEY_HASH and REQUIRE_AUTH=true.");
    }
    // If auth is required, a key hash must be present
    if (env.REQUIRE_AUTH && env.API_KEY_HASH.trim() === "") {
        throw new Error("[Config] REQUIRE_AUTH is true but API_KEY_HASH is not set.");
    }
...
// ---------------------------------------------------------------------------
function loadEnv() {
    const nodeEnv = assertNodeEnv(envString("NODE_ENV", "development"));
    const env = {
        NODE_ENV: nodeEnv,
        PORT: envInt("PORT", 4000),
        ALLOWED_ORIGINS: envList("ALLOWED_ORIGINS", nodeEnv === "production" ? [] : ["http://localhost:3000", "http://localhost:5173"]),
...
export function _reloadEnvForTesting() {
