...
import type { DomainEvent, DecisionResult, RuleTrace, RevenueEvent, RiskEvent } from "@lixeta/models";
...
function buildApiTrace(
  ...
  return {
    ...
    replayInstructions:
      "To reproduce this decision: POST /trigger-event with body " +
...
function buildHandshake(
  ...
  return {
    ...
    integrity: {
      description:
        "hash.txt contains SHA-256 of each evidence file and a combined manifest hash. " +
...
router.post("/", async (req: Request, res: Response): Promise<void> => {
  ...
  const ruleTraceDoc = buildRuleTrace(
    decision, traces, revenueEvents, riskEvents, aggressionLevel
  );
  const apiTraceDoc = buildApiTrace(sid, event, decision, sequenceNumber);
  const handshakeDoc = buildHandshake(
    sid, sessionCreatedAt, aggressionLevel,
    cfg.enabledRuleIds, event, env.NODE_ENV
  );
