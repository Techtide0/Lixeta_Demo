...
function generateSecureId(length: number): string {
  ...
  if (typeof g["crypto"] === "object" && g["crypto"] !== null) {
    const crypto = g["crypto"] as { getRandomValues(arr: Uint8Array): Uint8Array };
  ...
  if (process.env["NODE_ENV"] === "production") {
    throw new Error(
      "[SessionId] Web Crypto API unavailable in production. " +
 ...
 */
export function isValidSessionIdFormat(id: unknown): id is string {
  if (typeof id !== "string") return false;
  if (!id.startsWith("sess_")) return false;
