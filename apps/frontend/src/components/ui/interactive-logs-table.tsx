...
function FilterPanel({ filters, onChange, logs }: { filters: Filters; onChange: (f: Filters) => void; logs: Log[] }) {
  ...
  return (
    ...
    >
      ...
      {[{ label: "Level", category: "level" as const, items: levels }, { label: "Service", category: "service" as const, items: services }, { label: "Status", category: "status" as const, items: statuses }].map(({ label, category, items }) => (
...
export function InteractiveLogsTable({ logs: externalLogs }: InteractiveLogsTableProps = {}) {
  const [searchQuery, setSearchQuery] = useState("");
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [showFilters, setShowFilters] = useState(false);
  const [filters, setFilters] = useState<Filters>({ level: [], service: [], status: [] });

  const sourceLogs = externalLogs ?? [];
        ...
        <div className="space-y-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {filteredLogs.length} of {sourceLogs.length} entries
              {externalLogs && externalLogs.length > 0 && <span className="ml-2 text-green-400">● live</span>}
            </p>
          </div>
