...
export const SMART_NOTIFICATION_RULE_ID = "SMART_NOTIFICATION_V1" as const;
 ...
 */
function resolveAppOpen(ctx: EvaluationContext): boolean {
  const payload = ctx.event.payload as unknown as Record<string, unknown>;
  const appOpenValue = payload["appOpen"];
  if (typeof appOpenValue === "boolean") return appOpenValue;
  return false;
}
...
export const smartNotificationRule: Rule = {
  id: SMART_NOTIFICATION_RULE_ID,
  name: "Smart Notification",
  version: "1.2.0",
  description:
    "Suppresses SMS billing when the recipient has the app open, switching to " +
  ...
  evaluate(ctx: EvaluationContext): RuleEvaluationResult {
    ...
    if (wouldBeDeferred(ctx)) {
      const trace = buildTrace(
        ...
        "Message is outside active hours — will be deferred. No revenue impact recorded.",
        [
          {
    ...
    if (appOpen) {
      ...
      const trace: RuleTrace = buildTrace(
        traceId,
        ctx,
        "fired",
        `App is open — SMS suppressed. ₦${(SMS_COST_KOBO / 100).toFixed(2)} saved.`,
        conditions,
        actions
      );
      ...
      const trace: RuleTrace = buildTrace(
        ...
        ctx,
        "fired",
        "App is closed — SMS will be sent normally. Cost recorded for analytics.",
        conditions,
        actions
      );
