...
export const DIASPORA_RISK_RULE_ID = "DIASPORA_RISK_V1" as const;
...
function getEventCount(ctx: EvaluationContext, eventType: string): number {
  const counters = ctx.state.counters.eventCountByType as Record<string, number>;
...
export const diasporaRiskRule: Rule = {
  id: DIASPORA_RISK_RULE_ID,
  name: "Diaspora Risk",
  version: "1.0.0",
  description:
    "Detects high-velocity payment activity and repeated reversals within a " +
  ...
  applies(ctx: EvaluationContext): boolean {
    ...
    if (
      ctx.event.type === PAYMENT_INITIATED ||
  ...
  evaluate(ctx: EvaluationContext): RuleEvaluationResult {
    ...
    if (isUserEvent) {
      ...
      if (countryMismatch) {
        ...
        const trace = buildTrace(
          traceId, ctx, "fired",
          `FLAGGED: Geo anomaly on ${event.type} — device in ${deviceCountry}, account in ${accountCountry}.`,
          conditions, actions
        );
        ...
        return {
          ...
          verdictContribution: flagContribution(
            `Geo anomaly: login from ${deviceCountry} but account registered in ${accountCountry}`
          ),
        };
      }
      ...
      const trace = buildTrace(
        ...
        `User event with matching countries (${deviceCountry ?? "unknown"}). No geo risk.`,
        conditions, actions
      );
    ...
    const totalPaymentEvents =
      priorInitiated + priorSucceeded + priorReversed +
    ...
    const isReversalEscalation =
      event.type === PAYMENT_REVERSED && totalReversals >= REVERSAL_BLOCK_THRESHOLD;
    ...
    if (isReversalEscalation) {
      ...
      const trace = buildTrace(
        traceId, ctx, "fired",
        `BLOCKED: ${totalReversals} payment reversals in session. Possible chargeback fraud. Critical risk signal emitted.`,
        conditions, actions
      );

      return {
        trace,
        verdictContribution: blockContribution(
          `Repeated reversal pattern: ${totalReversals} reversals detected (threshold: ${REVERSAL_BLOCK_THRESHOLD})`
        ),
      };
    }
    ...
    if (isVelocityBreach) {
      ...
      const trace = buildTrace(
        traceId, ctx, "fired",
        `FLAGGED: ${totalPaymentEvents} payment events in session exceeds velocity threshold (${velocityThreshold}). Risk signal emitted.`,
        conditions, actions
      );
      ...
      return {
        trace,
        verdictContribution: flagContribution(
          `Payment velocity breach: ${totalPaymentEvents} events in session (threshold: ${velocityThreshold})`
        ),
      };
    }
    ...
    const trace = buildTrace(
      traceId, ctx, "no_match",
      `Payment within normal bounds — ${totalPaymentEvents} event(s) in session, ${totalReversals} reversal(s). No risk signals.`,
      conditions, actions
    );
