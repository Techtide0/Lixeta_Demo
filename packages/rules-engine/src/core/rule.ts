...
export type VerdictContribution =
  ...
  | { readonly type: "defer"; readonly until: string }
  | { readonly type: "no_opinion" }; // rule fired but has no verdict preference
...
export function flagContribution(reason: string): VerdictContribution {
  return { type: "flag", reason };
}

export function noOpinion(): VerdictContribution {
...
const VERDICT_PRIORITY: ReadonlyArray<VerdictContribution["type"]> = [
  "block",
  "flag",
  "transform",
  "defer",
...
export function mergeVerdicts(
  contributions: ReadonlyArray<VerdictContribution>
): VerdictContribution {
  if (contributions.length === 0) return { type: "allow" };

  let winner: VerdictContribution = { type: "no_opinion" };
...
export function outcomeFromContribution(
