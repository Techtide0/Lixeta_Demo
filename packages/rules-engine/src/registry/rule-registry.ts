...
const _rules: Map<string, Rule> = new Map();
const _order: string[] = [];
let _frozen = false;
...
export function registerRule(rule: Rule): void {
  if (_frozen) {
    throw new Error(
      `[RuleRegistry] Cannot register rule "${rule.id}" — registry is frozen. ` +
  ...
  if (_rules.has(rule.id)) {
    throw new Error(
      `[RuleRegistry] Duplicate rule ID detected: "${rule.id}". ` +
...
export function getEnabledRules(
  ...
  for (const id of enabledRuleIds) {
    if (!_rules.has(id)) {
      ...
      console.warn(
        `[RuleRegistry] Warning: enabled rule "${id}" is not registered. ` +
 ...
 */
export function listRegisteredRuleIds(): ReadonlyArray<string> {
  return [..._order];
}
