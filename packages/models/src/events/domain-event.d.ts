 ...
 */
export type EventChannel = "sms" | "whatsapp" | "email" | "push" | "in_app" | "api" | "internal" | "unknown";
 ...
 */
export interface DomainEvent<TType extends EventType = EventType, TPayload extends Record<string, unknown> = Record<string, unknown>> {
...
export type MessageSentEvent = TypedEvent<"message.sent", {
    messageId: string;
    recipientPhone: string;
}>;
export type MessageDeliveredEvent = TypedEvent<"message.delivered", {
    messageId: string;
}>;
export type MessageFailedEvent = TypedEvent<"message.failed", {
    messageId: string;
    reason: string;
}>;
export type MessageReadEvent = TypedEvent<"message.read", {
    messageId: string;
}>;
export type MessageRetriedEvent = TypedEvent<"message.retried", {
    messageId: string;
    attempt: number;
}>;
export type PaymentInitiatedEvent = TypedEvent<"payment.initiated", {
    paymentId: string;
    amount: number;
}>;
export type PaymentSucceededEvent = TypedEvent<"payment.succeeded", {
    paymentId: string;
}>;
export type PaymentFailedEvent = TypedEvent<"payment.failed", {
    paymentId: string;
    reason: string;
}>;
export type PaymentReversedEvent = TypedEvent<"payment.reversed", {
    paymentId: string;
}>;
export type PaymentTimeoutEvent = TypedEvent<"payment.timeout", {
    paymentId: string;
}>;
export type SessionStartedEvent = TypedEvent<"session.started", {
    sessionId: string;
    userId: string;
}>;
export type SessionEndedEvent = TypedEvent<"session.ended", {
    sessionId: string;
}>;
export type RuleFiredEvent = TypedEvent<"rule.fired", {
    ruleId: string;
    eventId: string;
}>;
export type RuleSkippedEvent = TypedEvent<"rule.skipped", {
    ruleId: string;
    reason: string;
}>;
export type RuleErrorEvent = TypedEvent<"rule.error", {
    ruleId: string;
    error: string;
}>;
export type SimulationStartedEvent = TypedEvent<"simulation.started", {
    simulationId: string;
}>;
export type SimulationCompletedEvent = TypedEvent<"simulation.completed", {
    simulationId: string;
}>;
export type SimulationResetEvent = TypedEvent<"simulation.reset", {
    simulationId: string;
}>;
